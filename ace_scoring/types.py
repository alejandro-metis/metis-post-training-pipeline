...
class CriterionResult:
    ...
    type: str
    score: int  # -1, 0, or +1
    stage_reached: str  # response_text | grounded_sources | response_text_only | link_verification
    hurdle_tag: str
    grounding_check: str
...
class TaskResult:
    ...
    task_id: str
    criteria_scores: list[list] = field(default_factory=list)  # [[score, type, hurdle_tag], ...]
    ...
    def to_reward(self, normalize: bool = True) -> float:
        ...
        if self._has_hurdles() and self.total_hurdle_score == 0:
            return 0.0
        if normalize:
            return self.total_score / self.num_criteria if self.num_criteria > 0 else 0.0
        return float(self.total_score)

    def _has_hurdles(self) -> bool:
        return any(
            len(cs) >= 3 and cs[2] == "Hurdle"
